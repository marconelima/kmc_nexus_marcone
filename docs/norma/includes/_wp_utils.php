...
function posts_per_page_selector($query) {
	...
	// Set posts per page
	$ppp = (int) get_theme_option('blog_posts_per_page');
	if ( $query->is_category() ) {
		$cat = (int) $query->get('cat');
		if (empty($cat))
			$cat = $query->get('category_name');
		if (!empty($cat)) {
			$ppp = (int) getCategoryInheritedProperty($cat, 'blog_posts_per_page', 0);
		}
	}
...
function showBreadcrumbs($args=array()) {
	...
	if ( !in_array($type, array('home', 'frontpage')) ) {
		...
		$depth = 0;
		$ex_cats = explode(',', get_theme_option('blog_exclude_cats'));
		do {
			if ($depth++ == 0) {
	...
	$rez .= '<ul class="breadcrumbs">'
		...
		. ($all_link && !in_array(my_strtolower($title), array('all posts')) ? '<li class="all"><a href="' . $all_link . '">' . sprintf( __( 'All %s', 'wpspace' ), $all_format ) . '</a></li>' : '' )
		. $rez2 
...
function getBlogTitle() {
	...
	if (  $page == 'template-blog.php' || is_page_template( 'template-blog.php' ) )
		...
		return sprintf(__('Author page: %s', 'wpspace'), $curauth->display_name);
...
function showPagination($args=array()) {
	...
	if (function_exists('wp_pagenavi') && !is_single()) {
		echo wp_pagenavi(array(
			'always_show' => 0,
			'style' => 1,
			'num_pages' => 5,
			'num_larger_page_numbers' => 3,
			'larger_page_numbers_multiple' => 10,
			'pages_text' => __('Page %CURRENT_PAGE% of %TOTAL_PAGES%', 'wpspace'),
			'current_text' => "%PAGE_NUMBER%",
			'page_text' => "%PAGE_NUMBER%",
			'first_text' => __('&laquo; First', 'wpspace'),
			'last_text' => __("Last &raquo;", 'wpspace'),
			'next_text' => "&raquo;",
			'prev_text' => "&laquo;",
			'dotright_text' => '', //"...",
			'dotleft_text' => '', //"...",
...
function showSinglePageNav( $nav_id ) {
	...
	<nav role="navigation" id="<?php echo esc_attr( $nav_id ); ?>" class="<?php echo $nav_class; ?>">
		...
		<?php if ( is_single() ) : // navigation links for single posts ?>
			<?php previous_post_link( '<div class="nav-previous">%link</div>', '<span class="meta-nav">' . __( '&larr;', 'wpspace' ) . '</span> %title' ); ?>
			<?php next_post_link( '<div class="nav-next">%link</div>', '%title <span class="meta-nav">' . __( '&rarr;', 'wpspace' ) . '</span>' ); ?>
...
add_filter('dynamic_sidebar_params', '_wp_utils_add_widget_number', 10, 1);
function _wp_utils_add_widget_number($prm) {
...
function getPostGallery($text, $id=0, $parse_text=true) {
	...
	$ids = array();
	if ($parse_text) {
		$ids_list = getTagAttrib($text, '[gallery]', 'ids');
		if ($ids_list!='') {
...
function substituteGallery($post_text, $post_id, $w, $h, $a='none') {
	$post_photos = false;
	while (($pos_start = my_strpos($post_text, '[gallery'))!==false) {
		$pos_end = my_strpos($post_text, ']', $pos_start);
		$tag_text = my_substr($post_text, $pos_start, $pos_end-$pos_start+1);
		if (($ids = getTagAttrib($tag_text, '[gallery]', 'ids'))!='') {
		...
		if (count($photos) > 0) {
			...
			foreach ($photos as $photo) {
				...
				$gallery_text .= '<li><a href="'. $photo . '" class="prettyPhoto">'.$photo_min.'</a></li>';
			}
			$gallery_text .= '
					</ul>
				</div>
		...
		$post_text = my_substr($post_text, 0, $pos_start) . $gallery_text . my_substr($post_text, $pos_end + 1);
	}
	return $post_text;
}
	...
	for ($i=0; $i<2; $i++) {
		...
		while (($pos_start = my_strpos($post_text, my_substr($tag, 0, -1).' ', $pos_start+1))!==false) {
			...
			if ($src != '') {
				...
				$post_text = my_substr($post_text, 0, $pos_start) 
					. '
						<div class="video_container' . ($tag_a ? ' align'.$tag_a : '') . '">
...
function showShareSocialLinks($args) {
	...
	foreach ($args['allowed'] as $s) {
		if (array_key_exists($s, $share_social_list)) {
			$link = str_replace(array('{id}', '{link}', '{title}', '{descr}'), array($args['post_id'], $args['post_link'], $args['post_title'], $args['post_descr']), $share_social_list[$s]);
...
$user_social_list = array(
	'facebook' => __('Facebook', 'wpspace'),
	'twitter' => __('Twitter', 'wpspace'),
	'gplus' => __('Google+', 'wpspace'),
	'linkedin' => __('LinkedIn', 'wpspace'),
	'dribbble' => __('Dribbble', 'wpspace'),
/*
	'pinterest' => __('Pinterest', 'wpspace'),
	'tumblr' => __('Tumblr', 'wpspace'),
	'behance' => __('Behance', 'wpspace'),
*/
	);
...
?>
	...
	<table class="form-table">
        <tr>
            <th><label for="user_position"><?php _e('User position', 'wpspace'); ?>:</label></th>
            <td><input type="text" name="user_position" id="user_position" size="55" value="<?php echo esc_attr(get_the_author_meta('user_position', $user->ID)); ?>" />
                <span class="description"><?php _e('Please, enter your position in the company', 'wpspace'); ?></span>
            </td>
        </tr>
    </table>
	...
	?>
        <tr>
            ...
            <td><input type="text" name="user_<?php echo $name; ?>" id="user_<?php echo $name; ?>" size="55" value="<?php echo esc_attr(get_the_author_meta('user_'.$name, $user->ID)); ?>" />
                <span class="description"><?php echo sprintf(__('Please, enter your %s link', 'wpspace'), $title); ?></span>
            </td>
        </tr>
	<?php
	}
...
function admin_taxonomy_filter() {
	...
	if (get_theme_option('admin_add_filters')=='yes') {
		$taxes[] = 'post_format';
		$taxes[] = 'post_tag';
	}
	$i = 0;
...
function getResizedImageTag( $post, $w=null, $h=null, $c=null, $u=true, $find_thumb=false ) {
	...
	$url = getResizedImageURL($post, $w, $h, $c, $u, $find_thumb);
	if ($url != '') {
