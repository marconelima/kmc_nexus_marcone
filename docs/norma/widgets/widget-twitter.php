...
class wpspace_twitter_widget extends WP_Widget {
	...
	function wpspace_twitter_widget() {
		/* Widget settings. */
		$widget_ops = array( 'classname' => 'widget_twitter', 'description' => __('Last Twitter Updates', 'wpspace') );

		/* Widget control settings. */
		$control_ops = array( 'width' => 200, 'height' => 250, 'id_base' => 'wpspace-twitter-widget' );

		/* Create the widget. */
		$this->WP_Widget( 'wpspace-twitter-widget', __('WP Space - Twitter timeline', 'wpspace'), $widget_ops, $control_ops );
	}
		...
		<div class="tweet widget_iframe_outer">
			...
			<script type="text/jscript">
				...
				function formatTwitter() {
					...
					var clink = '<?php echo get_theme_option('theme_color'); ?>';
					if (alink.length > 0) {
						alink.attr( {
...
<?php
	...
	function form( $instance ) {
		...
		$defaults = array( 'title' => '', 'description' => __('Last Twitter Updates', 'wpspace') );
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'twitter_count' ); ?>"><?php _e('Tweets count:', 'wpspace'); ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'twitter_username' ); ?>"><?php _e('Twitter Username:<br />(leave empty if you paste widget code)', 'wpspace'); ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'twitter_widget_id' ); ?>"><?php _e('Twitter Widget ID:<br />(leave empty if you paste widget code)', 'wpspace'); ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'twitter_code' ); ?>"><?php _e('or paste Twitter Widget Code:<br />(created in your profile on twitter.com)', 'wpspace'); ?></label>
